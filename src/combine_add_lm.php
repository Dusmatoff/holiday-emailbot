...
if(array_values($month_list_2) == array_intersect(array_values($month_list_2), $month)) {
	$contact_edit_param = array("id" => $id, "p[1]" => "1", "field[%CHECK_DATE%,0]" => '');
	$contact_edit = $ac->api("contact/edit", $contact_edit_param);
	...
	if(in_array($m_cy, $month)){
		...
		if($date[1] = $k_cy && $found_first_month_checked_by_subscriber == false){
			...
			if( $m_cy == 'Новый год' ){
				...
				$contact_edit_param = array("id" => $id, "p[1]" => "1", "field[%CHECK_DATE%,0]" => $check_date);
				$contact_edit = $ac->api("contact/edit", $contact_edit_param);
